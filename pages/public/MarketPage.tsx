...
import {
  ...
  MARKET_LINKS, marketIntelUrl, marketLinkByKey,
...
import EichlerIntel from '@/components/public/EichlerIntel'
...
export default function MarketPage() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      if (cfg.available && !cfg.southBay && !cfg.eichlerIntel) {
    ...
    <div className="mp-scope bg-white">
      ...
      <section className="max-w-6xl mx-auto px-6 pt-14 pb-2">
        <Reveal>
          ...
          {(() => {
            // Deep link straight into this market's own intelligence page for
            // visitors who want the full dataset rather than the hub summary.
            const link = marketLinkByKey(cfg.key)
            if (!link) return null
            return (
              <a
                href={marketIntelUrl(link)}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 mt-6 text-sm font-medium"
                style={{ color: BLUE }}
              >
                Full {cfg.shortName} market intelligence
      ...
      {cfg.eichlerIntel ? (
        <>
          <div style={{ background: NAVY_DEEP }} className="mt-8">
            <Marquee items={['Every recorded Eichler sale', 'Tract-level medians', '54 tracts tracked', 'One architecture, five cities', 'Updated continuously']} />
          </div>
          <EichlerIntel />
      ...
      {/* every market we operate — deep links to each one's intelligence page */}
      <section className="max-w-6xl mx-auto px-6 py-14 md:py-20">
        <Reveal>
          <div className="mp-mono text-[11px] uppercase tracking-[0.22em]" style={{ color: BLUE }}>
            Every market we operate
          </div>
          <h2 className="mp-serif text-[28px] md:text-[38px] leading-[1.08] font-semibold mt-3" style={{ color: NAVY }}>
            Go deeper on any market.
          </h2>
          <p className="mt-4 max-w-2xl leading-relaxed" style={{ color: INK }}>
            This page is the summary. Each marketplace publishes its own full intelligence — every
            building or street, every recorded sale, updated continuously.
          </p>
        </Reveal>
        <div className="mt-10 grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {MARKET_LINKS.map((m) => (
            <Reveal key={m.key}>
              <a
                href={marketIntelUrl(m)}
                target="_blank"
                rel="noopener noreferrer"
                className="mp-lift group block rounded-[22px] border border-black/[0.07] bg-white p-6 h-full"
              >
                <div className="flex items-start justify-between gap-3">
                  <h3 className="mp-serif text-lg font-semibold leading-snug" style={{ color: NAVY }}>{m.name}</h3>
                  <ExternalLink className="w-4 h-4 shrink-0 mt-1 opacity-40 group-hover:opacity-100 transition-opacity" style={{ color: NAVY }} />
                </div>
                <p className="text-[13px] mt-2 leading-relaxed" style={{ color: INK }}>{m.line}</p>
                <div className="mp-mono text-[10px] uppercase tracking-[0.16em] mt-4" style={{ color: BLUE }}>
                  {m.cta} →
                </div>
              </a>
            </Reveal>
